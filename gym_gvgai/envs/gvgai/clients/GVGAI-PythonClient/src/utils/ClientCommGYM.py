    ...
    * Client communication, set up the socket for a given agent
    ...
    def __init__(self, game, version, lvl, pathStr):
        ...
        cmd = [
            ...
            "-classpath",
            buildDir,
            "tracks.singleLearning.utils.JavaServer",
            "-game",
            game,
            "-gamesDir",
            gamesDir,
        ...
        if not os.path.isdir(buildDir):
            ...
            try:
                self.java = subprocess.Popen(
                    cmd, stdout=subprocess.DEVNULL, cwd=self.tempDir.name
                )
    ...
    def step(self, act):
        ...
        if (
            self.sso.isGameOver == True
            or self.sso.gameWinner == "PLAYER_WINS"
            or self.sso.phase == "FINISH"
            or self.sso.phase == "ABORT"
            or self.sso.phase == "End"
        ):
            self.sso.image = np.array(
                Image.open(
                    os.path.join(self.tempDir.name, "gameStateByBytes.png")
                ).convert("RGB")
            )
        ...
        if hasattr(self, "line"):
            ...
            if self.sso.Terminal:
                self.io.writeToServer(
                    self.lastMessageId, str(lvl) + "#" + self.lastSsoType, self.LOG
                )
            else:
        ...
        try:
            ...
            if self.sso.phase == "ACT":
                if (
                    ...
                    or self.lastSsoType == "IMAGE"
                    or self.lastSsoType == LEARNING_SSO_TYPE.BOTH
                    or self.lastSsoType == "BOTH"
                ):
                    if self.sso.imageArray:
                        self.sso.convertBytesToPng(
                            self.sso.imageArray, self.tempDir.name
                        )
                        self.sso.image = np.array(
                            Image.open(
                                os.path.join(self.tempDir.name, "gameStateByBytes.png")
    ...
    def start(self):
        ...
        if ect.exceededMaxTime():
            ...
            self.io.writeToServer(
                self.lastMessageId, "START_DONE" + "#" + self.lastSsoType, self.LOG
            )
    ...
    def init(self):
        ...
        if ect.exceededMaxTime():
            ...
            self.io.writeToServer(
                self.lastMessageId, "INIT_DONE" + "#" + self.lastSsoType, self.LOG
            )
    ...
    def act(self, action):
        ...
        if ect.exceededMaxTime():
            if ect.elapsedNanos() > CompetitionParameters.ACTION_TIME_DISQ * 1000000:
                ...
                self.io.writeToServer(
                    self.lastMessageId, "ACTION_NIL" + "#" + self.lastSsoType, self.LOG
                )
        else:
            self.io.writeToServer(
                self.lastMessageId, action + "#" + self.lastSsoType, self.LOG
            )
